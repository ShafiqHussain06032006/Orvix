...
DEFAULT_API_CONCURRENCIES = 5 #Default maximum number of concurrencies for a single model
model_semaphores: Dict[
    Tuple[str, str], asyncio.Semaphore
] = {}  # key: (model_name, platform)
openai_router = APIRouter(prefix="/v1", tags=["OpenAI compatible platform integration interface"])


@asynccontextmanager
async def get_model_client(model_name: str) -> AsyncGenerator[AsyncClient]:
    """
    To schedule the model with the same name, select: idle model -> The model with the least number of visits currently
    """
    max_semaphore = 0
    selected_platform = ""
...
async def list_models() -> Dict:
    """
    Integrated model list for all platforms.
    """
...
@openai_router.post("/audio/translations", deprecated="Not supported yet")
...
@openai_router.post("/audio/transcriptions", deprecated="Not supported yet")
...
@openai_router.post("/audio/speech", deprecated="Not supported yet")
